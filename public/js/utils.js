...
function getJumpFbid(fbid) {
    $("#JumpGame")[0].returnFbidToAs($('#footer').attr('data-fbid'), $('#footer').attr('data-env'));
}

// function requestFbId() {
//     FB.getLoginStatus(function(response) {
//         if (response.status === 'connected') {
//             YJIJ.fbid = response.authResponse.userID;
//             returnFbId();
...
{   
  ...
  if (response != null && response.to != null) {
    ...
    $("#JumpGame")[0].returnOpponentToAs(recipients[0]);
  } else {
...
function selectFriendsForLifeCallback(response)
{
    if (response != null && response.to != null) {
      ...
      $("#JumpGame")[0].returnFriendsForLifeCountToAs(inviteCount);
    } else {
      $("#JumpGame")[0].returnFriendsForLifeCountToAs(0);
    }
}
...
function sendLifeCallback(response)
{
    if (response != null && response.to != null) {
        var recipients = response.to;
        // $("#JumpGame")[0].returnFriendsForLifeToAs(recipients);
...
function purchaseCallback(response)
{
    ...
    if (response.status == 'completed') {
        ...
        $("#JumpGame")[0].returnPurchaseStatusToAs(signedRequest, paymentId);
    }
}
...
function inviteFriendsCallback(response) {
  if (response != null && response.to != null) {
    ...
    $("#JumpGame")[0].returnInviteCount(inviteCount);
  } else {
    $("#JumpGame")[0].returnInviteCount(0);
  }
}
...
function invitePreSelectedFriends(facebookIds) {
  ...
  //     $("#JumpGame")[0].returnProfilePictureUrlToAs(fbid, response.picture.data);
  //   }
...
function displayFlashScreenshot() {
  ...
  // var screenshotData = $("#JumpGame")[0].exportScreenshot();
